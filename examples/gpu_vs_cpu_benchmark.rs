...
async fn main() -> Result<()> {
    ...
    for (size_name, point_count) in test_sizes {
        println!("\n🔍 Dataset: {} ({} points)", size_name, format_number(point_count));
        ...
        println!("{:>8} ms ({:>10.0} pts/sec)", 
            format_duration(cpu_time), cpu_throughput);
        ...
        if let Some(gpu_renderer) = &mut gpu_renderer {
            print!("   GPU: ");
            let start = Instant::now();
            match gpu_renderer.transform_coordinates_optimal(
                ...
                Ok(_gpu_result) => {
                    ...
                    println!("{:>8} ms ({:>10.0} pts/sec) [{:.1}x speedup]", 
                        format_duration(gpu_time), gpu_throughput, speedup);
                },
                Err(e) => {
                    println!("Failed: {}", e);
                }
        ...
        println!("   Memory: {:.1} MB input → {:.1} MB output", 
            data_size as f64 / 1_000_000.0, result_size as f64 / 1_000_000.0);
    }
    ...
    println!("Total Capacity:  {}", format_number(stats.total_capacity() as u64));
...
async fn benchmark_line_plot(point_count: usize) -> Result<()> {
    println!("\n🔸 Line Plot Rendering ({} points)", format_number(point_count as u64));
    ...
    println!("   CPU Pipeline: {:>8} ms ({:>10.0} pts/sec)", 
        format_duration(cpu_total), throughput);
...
async fn benchmark_scatter_plot(point_count: usize) -> Result<()> {
    println!("\n🔸 Scatter Plot Rendering ({} points)", format_number(point_count as u64));
...
async fn benchmark_multiple_series(points_per_series: usize, series_count: usize) -> Result<()> {
    ...
    println!("\n🔸 Multi-Series Plot ({} series × {} = {} total points)", 
        series_count, format_number(points_per_series as u64), format_number(total_points as u64));
...
fn simulate_rendering_work(point_count: usize, complexity_factor: f64) {
    ...
    let work_duration = Duration::from_nanos(
        (point_count as f64 * complexity_factor * 10.0) as u64
    );
