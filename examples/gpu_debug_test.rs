...
async fn main() -> Result<()> {
    ...
    let mut gpu_renderer = match GpuRenderer::new().await {
        Ok(renderer) => {
            println!("✅ GPU Renderer created, threshold: {}", renderer.gpu_threshold());
            renderer
        },
        Err(e) => {
    ...
    let y_data: Vec<f64> = x_data.iter().map(|&x| (x * 2.0 * std::f64::consts::PI).sin()).collect();
    ...
    match gpu_renderer.transform_coordinates_optimal(
        ...
        Ok((x_result, y_result)) => {
            ...
            println!("✅ Success: {} ms, {} points transformed", 
                elapsed.as_millis(), x_result.len());
            println!("   First result: x={:.3}, y={:.3}", x_result[0], y_result[0]);
            println!("   Last result: x={:.3}, y={:.3}", 
                x_result[x_result.len()-1], y_result[y_result.len()-1]);
        },
        Err(e) => {
