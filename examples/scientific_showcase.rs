...
fn main() -> Result<()> {
    ...
    let sample_size = 500;
    let x_values: Vec<f64> = (0..sample_size).map(|i| {
    ...
    let distribution: Vec<f64> = (0..dist_samples).map(|i| {
    ...
    let group_size = 100;
    let group1: Vec<f64> = (0..group_size).map(|i| {
        8.0 + (i as f64 * 0.1).sin() * 2.0 + (i as f64 * 0.07).cos() * 1.0
    }).collect();
    let group2: Vec<f64> = (0..group_size).map(|i| {
        12.0 + (i as f64 * 0.08).cos() * 3.0 + (i as f64 * 0.05).sin() * 1.5
    }).collect();
    let group3: Vec<f64> = (0..group_size).map(|i| {
        15.0 + (i as f64 * 0.06).sin() * 1.5 + (i as f64 * 0.09).cos() * 2.5
    }).collect();
    ...
    println!("├─ Data generation: {} samples across 4 datasets", 
