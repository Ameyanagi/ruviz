...
fn generate_error_bars_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let experimental: Vec<f64> = theoretical.iter()
        .enumerate()
        .map(|(i, &y)| {
    ...
    let y_errors: Vec<f64> = experimental.iter()
        .map(|&y| y * 0.1 + 0.05)
        .collect();
...
fn generate_statistical_plots() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let normal_1: Vec<f64> = x_range.iter()
        .map(|&x| gaussian(x, 0.0, 1.0))
        .collect();
    
    let normal_2: Vec<f64> = x_range.iter()
        .map(|&x| gaussian(x, 1.0, 0.5))
        .collect();
    
    let normal_3: Vec<f64> = x_range.iter()
        .map(|&x| gaussian(x, -0.5, 1.5))
        .collect();
...
fn generate_publication_figure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let measurements: Vec<f64> = measurement_times.iter()
        .map(|&t| 10.0 * (-t * 0.1).exp() * (2.0 * PI * t * 0.3).cos() + 
                 0.3 * (t * 1.7).sin()) // Add some measurement noise
        .collect();
    let measurement_errors: Vec<f64> = measurements.iter()
        .map(|&m| m.abs() * 0.1 + 0.2)
        .collect();
