...
fn benchmark_coordinate_transformation(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000, 50000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("pooled", size), size, |b, &size| {
            b.iter(|| {
                let result = renderer
                    .transform_x_coordinates_pooled(
                        black_box(&test_data),
                        0.0,
                        size as f64 * 0.1,
                        0.0,
...
fn benchmark_memory_reuse_patterns(c: &mut Criterion) {
    ...
    group.bench_function("pooled_coordinate_reuse", |b| {
        b.iter(|| {
            ...
            for _ in 0..5 {
                let result = renderer
                    .transform_x_coordinates_pooled(black_box(&test_data), 0.0, 5000.0, 0.0, 800.0)
                    .unwrap();
                black_box(result);
...
fn benchmark_allocation_patterns(c: &mut Criterion) {
    ...
    group.bench_function("pooled_allocation_pattern", |b| {
        b.iter(|| {
            ...
            let _coords = renderer
                .transform_x_coordinates_pooled(&test_data, 0.0, 5000.0, 0.0, 800.0)
                .unwrap();
