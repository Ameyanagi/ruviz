...
The repository holds **three independent Cargo workspaces**, not one.

| Path | Workspace | Notes |
| --- | --- | --- |
| `/` (`ruviz`) | root | The core crate. Members: `bindings/wasm`, `bindings/python`; `default-members = ["."]`. |
| `bindings/wasm` (`ruviz-web`) | root | Browser/wasm adapter. |
| `bindings/python` (`ruviz-py`) | root | PyO3 bindings. |
| `adapters/gpui` (`ruviz-gpui`) | **its own** | Native GPUI adapter. Listed in the root `exclude`. |
| `adapters/gui` | **its own** | Virtual workspace for the egui, Iced, and Slint adapters. Listed in the root `exclude`. |

`adapters/gpui` is split out on purpose. It needs a `[patch.crates-io]`
...
- `-p ruviz-gpui` does not work from the repository root. Pass
  `--manifest-path adapters/gpui/Cargo.toml` instead, or run
  `make clippy-gpui`.
...
- The workspaces have separate lockfiles: `/Cargo.lock`,
  `adapters/gpui/Cargo.lock`, and `adapters/gui/Cargo.lock`.
...
drifting. Before it packages anything it asserts that `adapters/gpui` is
