...
- Public static raster outputs now follow the same reference renderer across `render()`, PNG export/save, and `render_to_renderer()`.
- The static raster backend now uses parity-safe CPU hot paths for dense scatter, line, and heatmap rendering.
...
See full details:
- [Release notes for v0.4.10](releases/v0.4.10.md)
...
```toml
[dependencies]
ruviz = { version = "0.4.10", features = ["typst-math"] }
```
...
```toml
[dependencies]
ruviz = { version = "0.4.10", features = ["polars_support"] }
polars = "0.35"
```
...
```toml
[dependencies]
ruviz = { version = "0.4.10", features = ["parallel"] }
```
...
```toml
[dependencies]
ruviz = { version = "0.4.10", features = ["parallel", "simd"] }
```
