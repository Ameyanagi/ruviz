...
```toml
[dependencies]
ruviz = { version = "0.3.1", features = ["ndarray_support", "parallel"] }
```
...
```toml
ruviz = "0.3.1"  # Includes: ndarray, parallel
```
...
ruviz = { version = "0.3.1", features = ["performance"] }

# Maximum capability (all features)
ruviz = { version = "0.3.1", features = ["full"] }
...
**Data Analysis**:
```toml
ruviz = { version = "0.3.1", features = ["polars_support", "performance"] }
```

**Publication Quality**:
```toml
ruviz = { version = "0.3.1", features = ["serde", "pdf"] }
```

**Real-time Visualization**:
```toml
ruviz = { version = "0.3.1", features = ["interactive-gpu"] }
```

**Large Datasets**:
```toml
ruviz = { version = "0.3.1", features = ["parallel", "simd", "gpu"] }
```
...
**Problem**: `error: package ruviz v0.3.1 cannot be built because it requires rustc 1.92 or newer`
...
```toml
ruviz = { version = "0.3.1", default-features = false, features = ["parallel"] }
```
