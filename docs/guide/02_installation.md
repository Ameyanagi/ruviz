...
```toml
[dependencies]
ruviz = { version = "0.3.3", features = ["ndarray_support", "parallel"] }
```
...
```toml
ruviz = "0.3.3"  # Includes: ndarray, parallel
```
...
ruviz = { version = "0.3.3", features = ["performance"] }

# Maximum capability (all features)
ruviz = { version = "0.3.3", features = ["full"] }

# Minimal (no default features)
ruviz = { version = "0.3.3", default-features = false }
```
...
**Data Analysis**:
```toml
ruviz = { version = "0.3.3", features = ["polars_support", "performance"] }
```

**Publication Quality**:
```toml
ruviz = { version = "0.3.3", features = ["serde", "pdf"] }
```

**Real-time Visualization**:
```toml
ruviz = { version = "0.3.3", features = ["interactive-gpu"] }
```

**Large Datasets**:
```toml
ruviz = { version = "0.3.3", features = ["parallel", "simd", "gpu"] }
```
...
**Problem**: `error: package ruviz v0.3.3 cannot be built because it requires rustc 1.92 or newer`
...
```toml
ruviz = { version = "0.3.3", default-features = false, features = ["parallel"] }
```
