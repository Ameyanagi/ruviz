...
## Fixed proportions during rotation

Use `axis_aspect` to choose X:Y:Z box proportions and `stable_scale(true)` to
keep the projection and viewport fixed during orbit. For example, a 1:2:1 box:

```rust,check,features=3d
use ruviz::prelude::*;

fn main() -> PlotResult<()> {
    line3d(&[0.0, 1.0], &[0.0, 1.0], &[0.0, 1.0])
        .axis_aspect(AxisAspect3D::fixed(1.0, 2.0, 1.0))
        .stable_scale(true)
        .save("fixed_ratio.png")
}
```

`AxisAspect3D::Data` instead gives one data unit the same physical length on
every axis. Use it for molecules and other geometry whose shape must be
preserved. `Equal` makes the three axis lengths equal even when their data
ranges differ; `Fixed` sets those lengths to your ratio. Ratios are positive
and finite, and apply before camera projection. Foreshortening during rotation
is expected; rotation does not rescale the data independently on each axis. Equal-data
scaling preserves small nonzero ratios; ranges that cannot be represented by
the renderer return a validation error instead of stretching an axis.

Stable scale reserves room for every orientation, leaving more space around
the scene than the default tight fit. Orbit and roll preserve its framing;
explicit zoom, pan, resize, or changes to data bounds can still change the view.
The sphere factory enables equal data units and stable scale by default.
Replacing the entire camera also replaces those defaults.

The web SDK exposes the same controls as `axisAspect(1, 2, 1)`, `equalScale()`,
and `stableScale()`. For example:

```javascript
const session = await line3d(x, y, z)
  .axisAspect(1, 2, 1)
  .stableScale()
  .mount(canvas);
```
...
Use `spheres3d(&atoms)` for physical atom radii, per-instance colors and opacity,
stable pick IDs, and optional shading. See [Molecular spheres](13_molecular_views.md)
for the API, GPUI toggle, and transparency/export policy.

For a molecular or spatial view, `.axes(false)` hides the box, panes, grid,
ticks, and axis labels. A compact orientation cue, title, and legend remain.
The default `.axes(true)` preserves the standard plot presentation.
