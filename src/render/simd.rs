...
mod disabled {
    use crate::core::types::{Point2f, Transform2D, BoundingBox};
    ...
    impl SIMDTransformer {
        ...
        pub fn transform_coordinates_scalar(&self, points: &[(f64, f64)], scale: f32, offset: f32) -> Vec<Point2f> {
            points.iter().map(|&(x, y)| {
                Point2f::new((x as f32) * scale + offset, (y as f32) * scale + offset)
            }).collect()
        }
        
        pub fn transform_coordinates_2d_scalar(&self, points: &[(f64, f64)], x_scale: f32, y_scale: f32, x_offset: f32, y_offset: f32) -> Vec<Point2f> {
            points.iter().map(|&(x, y)| {
                Point2f::new((x as f32) * x_scale + x_offset, (y as f32) * y_scale + y_offset)
            }).collect()
        }
        
        pub fn distance_from_point_scalar(&self, points: &[Point2f], reference: Point2f) -> Vec<f32> {
            points.iter().map(|p| {
        ...
        pub fn apply_transform_scalar(&self, points: &[Point2f], transform: &Transform2D) -> Vec<Point2f> {
            points.iter().map(|p| {
                Point2f::new(
...
impl SIMDTransformer {
    ...
    pub fn batch_distance_squared_simd(
        &self,
        points: &[Point2f],
        reference: Point2f,
    ) -> Vec<f32> {
    ...
    fn batch_distance_squared_scalar(
        &self,
        points: &[Point2f],
        reference: Point2f,
    ) -> Vec<f32> {
...
pub struct AffineTransform2D {
    pub m11: f32, pub m12: f32, pub tx: f32,
    pub m21: f32, pub m22: f32, pub ty: f32,
}
...
impl AffineTransform2D {
    ...
    pub fn identity() -> Self {
        Self {
            m11: 1.0, m12: 0.0, tx: 0.0,
            m21: 0.0, m22: 1.0, ty: 0.0,
        }
    }
    ...
    pub fn scale(scale: f32) -> Self {
        Self {
            m11: scale, m12: 0.0, tx: 0.0,
            m21: 0.0, m22: scale, ty: 0.0,
        }
    }
    ...
    pub fn rotation(angle: f32) -> Self {
        ...
        let sin_a = angle.sin();
        Self {
            m11: cos_a, m12: -sin_a, tx: 0.0,
            m21: sin_a, m22: cos_a, ty: 0.0,
        }
    }
}
...
mod tests {
    ...
    fn test_coordinate_transformation_consistency() {
        ...
        let simd_result = transformer.transform_coordinates_simd(&x_data, &y_data, bounds.clone(), viewport.clone()).unwrap();
        let scalar_result = transformer.transform_coordinates_scalar(&x_data, &y_data, bounds, viewport);
