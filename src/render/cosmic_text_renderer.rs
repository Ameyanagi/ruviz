use cosmic_text::{
    Attrs, Buffer, Color as CosmicColor, Family, FontSystem, Metrics, Shaping, SwashCache, Weight, Stretch, Style
};
use tiny_skia::{Pixmap, PremultipliedColorU8};
use crate::{
    core::error::{Result, PlottingError},
    render::Color,
};
...
impl CosmicTextRenderer {
    ...
    pub fn render_text(
        ...
        let attrs = Attrs::new()
            .family(Family::Name("Roboto"));  // Use Roboto font directly
                ...
                self.swash_cache.with_pixels(
                    ...
                    |x, y, color| {
                        ...
                        if pixel_x >= 0 && pixel_y >= 0 
                            ...
                            let alpha = color.a();
                            if alpha > 0 {
                                let pixmap_idx = (pixel_y as u32 * pixmap.width() + pixel_x as u32) as usize;
                                ...
                                let blended_r = (color.r() as f32 * alpha_f + background.red() as f32 * inv_alpha) as u8;
                                let blended_g = (color.g() as f32 * alpha_f + background.green() as f32 * inv_alpha) as u8;
                                let blended_b = (color.b() as f32 * alpha_f + background.blue() as f32 * inv_alpha) as u8;
    ...
    pub fn render_text_rotated(
        ...
        // Add padding for glyph rendering margins (cosmic-text dx/dy can be negative)
        let padding = 20.0; // Extra space for glyph rendering
        min_x -= padding;
        min_y -= padding;
        max_x += padding;
        max_y += padding;
        ...
        let mut rotated_pixmap = Pixmap::new(rotated_width, rotated_height)
            .ok_or_else(|| PlottingError::RenderError("Failed to create rotated pixmap".to_string()))?;
        rotated_pixmap.fill(tiny_skia::Color::TRANSPARENT);
        
        for orig_y in 0..text_height {
            for orig_x in 0..text_width {
                let src_pixel = temp_pixmap.pixels()[orig_y as usize * text_width as usize + orig_x as usize];
        ...
        let target_x = x as i32 - (rotated_width / 2) as i32;
        let target_y = y as i32 - (rotated_height / 2) as i32;
        
        for py in 0..rotated_height {
            for px in 0..rotated_width {
                let src_pixel = rotated_pixmap.pixels()[py as usize * rotated_width as usize + px as usize];
                if src_pixel.alpha() > 0 {
                    let final_x = target_x + px as i32;
                    let final_y = target_y + py as i32;
                    
                    if final_x >= 0 && final_y >= 0 && 
                       final_x < pixmap.width() as i32 && final_y < pixmap.height() as i32 {
                        
                        let pixmap_idx = (final_y as u32 * pixmap.width() + final_x as u32) as usize;
