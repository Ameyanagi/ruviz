...
impl Position {
    ...
    pub fn opposite(self) -> Self {
        match self {
            ...
            Position::Custom { x, y } => Position::Custom {
                x: 1.0 - x,
                y: 1.0 - y,
            },
        }
    ...
    pub fn get_recommended_margin(self) -> (f32, f32) {
        ...
        match self {
            ...
            | Position::TopRight
            | Position::BottomLeft
            | Position::BottomRight => (CORNER_MARGIN, CORNER_MARGIN),
...
mod tests {
    ...
    fn test_normalized_coordinates() {
        ...
        assert_eq!(
            Position::custom(0.25, 0.75).to_normalized_coords(),
            (0.25, 0.75)
        );

        // Test clamping
        assert_eq!(
            Position::custom(-0.5, 1.5).to_normalized_coords(),
            (0.0, 1.0)
        );
    }
    ...
    fn test_pixel_coordinates() {
        ...
        assert_eq!(
            Position::BottomRight.to_pixel_coords(800, 600),
            (800.0, 600.0)
        );
    }
    ...
    fn test_opposite_positions() {
        ...
        assert_eq!(
            Position::custom(0.25, 0.75).opposite(),
            Position::custom(0.75, 0.25)
        );
    }
    ...
    fn test_display() {
        ...
        assert_eq!(
            Position::custom(0.25, 0.75).to_string(),
            "custom(0.25, 0.75)"
        );
    }
}
