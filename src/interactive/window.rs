...
use crate::{
    ...
    interactive::{
        event::{InteractionEvent, EventHandler, EventProcessor, RawInputEvent, MouseButton, Point2D, Vector2D},
...
use winit::{
    ...
    event::{Event, WindowEvent, ElementState, MouseScrollDelta, MouseButton as WinitMouseButton},
...
impl InteractiveWindow {
    ...
    pub async fn new(
        ...
        interaction_state.data_bounds = crate::interactive::event::Rectangle::new(0.0, 0.0, 100.0, 100.0);
        interaction_state.screen_bounds = crate::interactive::event::Rectangle::new(0.0, 0.0, width as f64, height as f64);
    ...
    pub fn run(mut self, plot: Plot) -> Result<()> {
        ...
        event_loop.run_app(&mut app_handler).map_err(|e| {
            PlottingError::RenderError(format!("Event loop error: {}", e))
        })?;
        ...
        match event {
            ...
            WindowEvent::MouseInput { state, button, .. } => {
                ...
                if pressed {
                    ...
                    if let Some(interaction_event) = EventProcessor::process_raw_event(raw_event, &self.interaction_state) {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                let button_pressed = self.mouse_buttons_pressed
                    .values()
                    .any(|&pressed| pressed);
                ...
                if let Some(interaction_event) = EventProcessor::process_raw_event(raw_event, &self.interaction_state) {
                    ...
                    if let Some(data_point) = self.renderer.get_data_point_at(hover_pos, &self.interaction_state) {
                        self.interaction_state.update_hover(hover_pos, Some(data_point));
                    } else {
    ...
    fn process_interaction_event(&mut self, event: InteractionEvent) -> Result<()> {
        ...
        match &event {
            ...
            InteractionEvent::SelectPoint { point } => {
                if let Some(data_point) = self.renderer.get_data_point_at(*point, &self.interaction_state) {
            ...
            InteractionEvent::Hover { point } => {
                if let Some(data_point) = self.renderer.get_data_point_at(*point, &self.interaction_state) {
                    self.interaction_state.update_hover(*point, Some(data_point));
                } else {
            ...
            InteractionEvent::ShowTooltip { content, position } => {
                self.interaction_state.show_tooltip(content.clone(), *position);
            }
...
impl EventHandler for DefaultEventHandler {
    fn handle_event(&mut self, event: InteractionEvent) -> Result<crate::interactive::event::EventResponse> {
...
impl InteractiveWindowBuilder {
    ...
    pub async fn build(self, plot: Plot) -> Result<InteractiveWindow> {
        let mut window = InteractiveWindow::new(
            plot,
            &self.title,
            self.width,
            self.height,
...
mod tests {
    ...
    fn test_mouse_button_conversion() {
        ...
        assert_eq!(window.convert_mouse_button(WinitMouseButton::Left), MouseButton::Left);
        assert_eq!(window.convert_mouse_button(WinitMouseButton::Right), MouseButton::Right);
        assert_eq!(window.convert_mouse_button(WinitMouseButton::Middle), MouseButton::Middle);
    }
    ...
    fn test_default_event_handler() {
        ...
        assert_eq!(response.unwrap(), crate::interactive::event::EventResponse::Handled);
