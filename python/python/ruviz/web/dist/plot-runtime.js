export function normalizeBackendPreference(backendPreference) {
...
export function toRawBackendPreference(module, backendPreference) {
    switch (backendPreference) {
        case "cpu":
...
function toObservable(module, source) {
    return new module.ObservableVecF64(Float64Array.from(source.values));
}
export function applySnapshotMetadata(rawPlot, snapshot) {
    if (snapshot.sizePx) {
...
export function buildRawPlotFromSnapshot(snapshot, module) {
        ...
        switch (series.kind) {
            ...
            case "scatter": {
                if (series.y.kind === "sine-signal") {
                    const signal = module.SignalVecF64.sineWave(series.y.options.points, series.y.options.domainStart, series.y.options.domainEnd, series.y.options.amplitude, series.y.options.cycles, series.y.options.phaseVelocity, series.y.options.phaseOffset, series.y.options.verticalOffset);
            ...
            case "error-bars": {
                if (series.x.kind === "observable" ||
                    ...
                    rawPlot.error_bars_observable(toObservable(module, series.x), toObservable(module, series.y), toObservable(module, series.yErrors));
                }
                else {
            ...
            case "error-bars-xy": {
                if (series.x.kind === "observable" ||
                    ...
                    series.yErrors.kind === "observable") {
                    rawPlot.error_bars_xy_observable(toObservable(module, series.x), toObservable(module, series.y), toObservable(module, series.xErrors), toObservable(module, series.yErrors));
                }
                else {
...
//# sourceMappingURL=plot-runtime.js.map