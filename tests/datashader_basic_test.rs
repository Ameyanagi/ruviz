...
fn test_datashader_basic_functionality() {
    ...
    println!(
        ...
        stats.canvas_utilization * 100.0
    );
}
...
fn test_datashader_large_dataset() {
    ...
    let y_data: Vec<f64> = x_data
        .iter()
        .map(|&x| (x * 2.0).sin() + 0.5 * (x * 10.0).cos())
        .collect();
    ...
    assert!(
        result.is_ok(),
        "DataShader aggregation should succeed with large dataset"
    );
    ...
    assert!(
        stats.filled_pixels > 100,
        "Should have many filled pixels for large dataset"
    );
...
fn test_plot_datashader_activation() {
    ...
    assert!(
        result.is_ok(),
        "Plot with large dataset should render successfully using DataShader"
    );
