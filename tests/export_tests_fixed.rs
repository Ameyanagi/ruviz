...
fn test_raw_data_exports() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let metadata1 = format!(
        ...
        image1.width, image1.height, image1.pixels.len()
    );
    ...
    let metadata2 = format!(
        "Small Plot Raw Data\nSize: {}x{}\nBytes: {}\nFormat: RGBA",
        image2.width, image2.height, image2.pixels.len()
    );
    ...
    let metadata3 = format!(
        "Bar Chart Raw Data\nSize: {}x{}\nBytes: {}\nFormat: RGBA\nCategories: {}",
        image3.width, image3.height, image3.pixels.len(), categories.len()
    );
...
fn test_all_themes_export() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for (theme_name, theme) in themes {
        ...
        fs::write(&format!("export_output/raw/theme_{}.bin", theme_name), &image.pixels)?;
    }
...
fn test_different_resolutions() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for (width, height, name) in resolutions {
        ...
        Plot::new()
            ...
            .line(&x_data, &y_data)
            .end_series()
            .save(&format!("export_output/png/resolution_{}_{}_{}x{}.png", name, "png", width, height))?;
        
        // SVG
        let renderer = SkiaRenderer::new(width, height, Theme::default())?;
        renderer.export_svg(
            &format!("export_output/svg/resolution_{}_{}_{}x{}.svg", name, "svg", width, height),
            width,
            height
        )?;
        ...
        fs::write(
            &format!("export_output/raw/resolution_{}_{}x{}.bin", name, width, height),
            &image.pixels
        )?;
        ...
        fs::write(
            &format!("export_output/raw/resolution_{}_{}x{}_info.txt", name, width, height),
            info
        )?;
    }
...
fn test_export_validation() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(non_zero_pixels > 1000, "Image should contain substantial non-zero pixel data");
    
    // Check for color variation (not monochrome)
    let unique_values: std::collections::HashSet<&u8> = image.pixels.iter().collect();
    assert!(unique_values.len() > 10, "Image should have color variation");
