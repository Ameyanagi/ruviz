...
fn test_basic_scatter_plot() {
    ...
    assert!(result.is_ok(), "Basic scatter plot should export successfully");
    
    let path = std::path::Path::new("test_output/basic_scatter.png");
    assert!(path.exists(), "Output file should exist");
    
    let metadata = fs::metadata(path).unwrap();
    assert!(metadata.len() > 1000, "PNG file should have reasonable size");
}
...
fn test_scatter_plot_different_dpi() {
    ...
    let filenames = vec!["scatter_96_test.png", "scatter_150_test.png", "scatter_300_test.png", "scatter_600_test.png"];
    ...
    for (dpi, filename) in dpi_values.iter().zip(filenames.iter()) {
        ...
        assert!(result.is_ok(), "Scatter plot at {} DPI should export successfully", dpi);
        ...
        let metadata = fs::metadata(path).unwrap();
        assert!(metadata.len() > 1000, "PNG file should have reasonable size for {} DPI", dpi);
    }
}
...
fn test_scatter_plot_with_custom_colors() {
    ...
    assert!(result.is_ok(), "Scatter plot with custom color should export successfully");
}
...
fn test_multiple_scatter_series() {
    ...
    assert!(result.is_ok(), "Multiple scatter series should export successfully");
}
...
fn test_scatter_plot_large_dataset() {
    ...
    assert!(result.is_ok(), "Large dataset scatter plot should export successfully");
}
...
fn test_scatter_plot_empty_data() {
    ...
    assert!(result.is_err(), "Empty data should return an error when saving");
}
...
fn test_scatter_plot_mismatched_data() {
    ...
    assert!(result.is_err(), "Mismatched data lengths should return an error");
}