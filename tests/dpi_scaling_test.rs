...
fn test_dpi_scaling_produces_different_file_sizes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    base_plot.clone().dpi(96).save("test_output/dpi_scaling_96_test.png")?;    // Screen
    base_plot.clone().dpi(150).save("test_output/dpi_scaling_150_test.png")?;  // Web  
    base_plot.clone().dpi(300).save("test_output/dpi_scaling_300_test.png")?;  // Print
    base_plot.clone().dpi(600).save("test_output/dpi_scaling_600_test.png")?;  // IEEE
    ...
    println!("File sizes - 96 DPI: {} bytes, 150 DPI: {} bytes, 300 DPI: {} bytes, 600 DPI: {} bytes",
             size_96, size_150, size_300, size_600);
    
    // Assert that higher DPI produces larger files (due to larger canvas)
    assert!(size_150 > size_96, "150 DPI should be larger than 96 DPI: {} vs {}", size_150, size_96);
    assert!(size_300 > size_150, "300 DPI should be larger than 150 DPI: {} vs {}", size_300, size_150);
    assert!(size_600 > size_300, "600 DPI should be larger than 300 DPI: {} vs {}", size_600, size_300);
    
    // Verify significant size differences (not just compression artifacts)
    let ratio_300_to_96 = size_300 as f64 / size_96 as f64;
    assert!(ratio_300_to_96 > 2.0, "300 DPI should be significantly larger than 96 DPI, ratio: {}", ratio_300_to_96);
    
    println!("✓ DPI scaling produces correctly sized images: 300/96 ratio = {:.1}x", ratio_300_to_96);
    Ok(())
}
...
fn test_dpi_canvas_size_scaling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    plot.clone().dpi(96).save("test_output/canvas_96_test.png")?;
    plot.clone().dpi(192).save("test_output/canvas_192_test.png")?;
    plot.clone().dpi(288).save("test_output/canvas_288_test.png")?;
    ...
    println!("Canvas sizes - 96 DPI: {} bytes, 192 DPI: {} bytes, 288 DPI: {} bytes", 
    ...
    assert!(ratio_2x > 2.0, "2x DPI should produce significantly larger files, got ratio: {:.1}", ratio_2x);
    assert!(ratio_3x > 3.0, "3x DPI should produce much larger files, got ratio: {:.1}", ratio_3x);
    
    println!("✓ Canvas scaling works: 2x DPI = {:.1}x size, 3x DPI = {:.1}x size", ratio_2x, ratio_3x);
    Ok(())
}
...
fn test_dpi_font_scaling_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    plot.clone().dpi(300).save("test_output/font_300_test.png")?;
    ...
    assert!(font_scaling_ratio > 3.0, "Font scaling should increase file size significantly: {:.1}x", font_scaling_ratio);
    
    println!("✓ Font scaling contributes to DPI scaling: {:.1}x size increase", font_scaling_ratio);
    Ok(())
}