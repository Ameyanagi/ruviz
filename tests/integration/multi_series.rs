...
mod multi_series_integration_tests {
    ...
    fn test_story_2_multi_series_integration() {
        ...
        assert!(
            result.is_ok(),
            "Multi-series plot creation failed: {:?}",
            result.err()
        );

        // Verify the file was created
        assert!(
            Path::new("test_multi_series_integration.png").exists(),
        ...
        assert!(
            metadata.len() > 1000,
            "Multi-series PNG seems too small: {} bytes",
            metadata.len()
        );
    ...
    fn test_automatic_color_management_integration() {
        ...
        assert!(
            result.is_ok(),
            "Automatic color cycling failed: {:?}",
            result.err()
        );
    ...
    fn test_many_series_integration() {
        ...
        for i in 0..10 {
            ...
            let y: Vec<f64> = x.iter().map(|&x| amplitude * (x + phase).sin()).collect();

            plot = plot.line(&x, &y).label(&format!("Series {}", i + 1));
        }
