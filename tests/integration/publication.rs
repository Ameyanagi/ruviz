...
mod publication_integration_tests {
    ...
    fn test_story_4_publication_quality_integration() {
        ...
        assert!(
            result_png.is_ok(),
            "Publication PNG export failed: {:?}",
            result_png.err()
        );

        // Verify PNG file was created with appropriate properties
        assert!(
            Path::new("test_publication_integration.png").exists(),
        ...
        assert!(
            result_svg.is_ok(),
            "Publication SVG export failed: {:?}",
            result_svg.err()
        );

        // Verify SVG file
        assert!(
            Path::new("test_publication_integration.svg").exists(),
        ...
        assert!(
            svg_metadata.len() > 1000,
            "Publication SVG too small: {} bytes",
            svg_metadata.len()
        );
    ...
    fn test_high_resolution_export_integration() {
        ...
        for &dpi in &dpi_settings {
            let result = plot.clone().dpi(dpi).save(&format!("test_{}dpi.png", dpi));
    ...
    fn test_scientific_formatting_integration() {
        ...
        assert!(
            result1.is_ok(),
            "Scientific notation (large) failed: {:?}",
            result1.err()
        );
        ...
        assert!(
            result2.is_ok(),
            "Scientific notation (small) failed: {:?}",
            result2.err()
        );
    ...
    fn test_colorblind_friendly_integration() {
        ...
        assert!(
            result.is_ok(),
            "Color-blind friendly theme failed: {:?}",
            result.err()
        );
    ...
    fn test_print_optimization_integration() {
        ...
        assert!(
            result2.is_ok(),
            "Letter print optimization failed: {:?}",
            result2.err()
        );
    ...
    fn test_format_consistency_integration() {
        ...
        assert!(
            png_result.is_ok(),
            "PNG consistency export failed: {:?}",
            png_result.err()
        );
        assert!(
            svg_result.is_ok(),
            "SVG consistency export failed: {:?}",
            svg_result.err()
        );
        ...
        println!(
            "Format consistency - PNG: {} KB, SVG: {} KB",
            png_size / 1000,
            svg_size / 1000
        );
